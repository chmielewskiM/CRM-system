...
import { TaskNotification } from '../taskNotifier/TaskNotification';

export const DelegatedTaskDetails: React.FC = () => {
  const rootStore = useContext(RootStoreContext);
  const { loadingInitial, selectedTask, selectTask, rr } = rootStore.delegatedTaskStore;
          ...
          <Grid>
            <Grid.Column>
              ...
              <Grid.Row>
                ...
                <span className="detail-value">{selectedTask?.access.createdBy}</span>
              </Grid.Row>
              <Grid.Row>
                <span className="detail">Shared with: </span>{' '}
                <span className="detail-value">{selectedTask?.access.sharedWith}</span>
              </Grid.Row>
              <Grid.Row>
              ...
              <Grid.Row>
                ...
                <span className="detail-value">{destructureDate(new Date(selectedTask!.deadline))}</span>
              </Grid.Row>

              {selectedTask?.finishedBy != '' && (
                <Grid.Row>
                  <span className="detail">Finished by: </span>{' '}
                  <span className="detail-value">{selectedTask?.finishedBy}</span>
                </Grid.Row>
              )}
              <Grid.Row>
